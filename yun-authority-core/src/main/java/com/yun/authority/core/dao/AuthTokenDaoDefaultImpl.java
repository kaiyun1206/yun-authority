 ...
 */
public class AuthTokenDaoDefaultImpl implements AuthTokenDao {
     ...
     * 构造函数
     */
    public AuthTokenDaoDefaultImpl() {
        initRefreshThread();
    }
    ...
    public void set(String key, String value, long timeout) {
        if(timeout == 0 || timeout <= AuthTokenDao.NOT_VALUE_EXPIRE)  {
            return;
        }
        dataMap.put(key, value);
        expireMap.put(key, (timeout == AuthTokenDao.NEVER_EXPIRE) ? (AuthTokenDao.NEVER_EXPIRE) : (System.currentTimeMillis() + timeout * 1000));
    }

    @Override
    public void update(String key, String value) {
        if(getKeyTimeout(key) == AuthTokenDao.NOT_VALUE_EXPIRE) {
            return;
        }
        dataMap.put(key, value);
    }
    ...
    void clearKeyByTimeout(String key) {
        ...
        if(expirationTime != null && expirationTime != AuthTokenDao.NEVER_EXPIRE && expirationTime < System.currentTimeMillis()) {
            dataMap.remove(key);
            expireMap.remove(key);
        }
    }
    ...
    long getKeyTimeout(String key) {
        ...
        // 如果被标注为永不过期
        if(expire == AuthTokenDao.NEVER_EXPIRE) {
    ...
    public void initRefreshThread() {
        ...
        // 如果配置了<=0的值，则不启动定时清理
        if(AuthManager.getConfig().getDataRefreshPeriod() <= 0) {
            return;
        }
        // 启动定时刷新
        ...
        this.refreshThread = new Thread(() -> {
            for (;;) {
                try {
                    ...
                    // 休眠N秒
                    int dataRefreshPeriod = AuthManager.getConfig().getDataRefreshPeriod();
    ...
    public List<String> searchData(String prefix, String keyword, int start, int size) {
        return AuthFoxUtil.searchList(expireMap.keySet(), prefix, keyword, start, size);
    }
