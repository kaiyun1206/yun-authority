...
public class AuthSSOHandler {
    ...
    public static Object serverRequest() {
        ...
        // 默认返回 
        return AuthSSOConstants.NOT_HANDLE;
    }
    ...
    public static Object ssoAuth() {
        ...
        AuthSSOConfig cfg = AuthManager.getConfig().getSSO();
        AuthLogic authLogic = AuthSSOUtil.authSSOTemplate.authLogic;

        // ---------- 此处两种情况分开处理：
        // 情况1：在SSO认证中心尚未登录，则先去登登录 
        if(authLogic.isLogin() == false) {
            return cfg.notLoginView.get();
        }
        // 情况2：在SSO认证中心已经登录，开始构建授权重定向地址，下放ticket 
        String redirectUrl = AuthSSOUtil.buildRedirectUrl(authLogic.getLoginId(), req.getParam(ParamName.redirect));
    ...
    public static Object ssoCheckTicket() {
        ...
        // 校验ticket，获取对应的账号id 
        Object loginId = AuthSSOUtil.checkTicket(ticket);

        // 注册此客户端的单点注销回调URL 
        AuthSSOUtil.registerSloCallbackUrl(loginId, sloCallback);
    ...
    public static Object ssoServerLogout() {
        ...
        // step.1 校验秘钥 
        AuthSSOUtil.checkSecretkey(secretkey);

        // step.2 遍历通知Client端注销会话 
        AuthSSOUtil.forEachSloUrl(loginId, url -> cfg.sendHttp.apply(url));

        // step.3 Server端注销 
        authLogic.logoutByTokenValue(authLogic.getTokenValueByLoginId(loginId));

        // 完成
        return AuthSSOConstants.OK;
    }
    ...
    public static Object ssoLogin() {
         ...
         */
        if(ticket == null) {
            String serverAuthUrl = AuthSSOUtil.buildServerAuthUrl(AuthHolder.getRequest().getUrl(), back);
            ...
            if(cfg.isHttp) {
                ...
                if(cfg.isSlo) {
                    ssoLogoutCall = AuthHolder.getRequest().getUrl().replace(Api.ssoLogin, Api.ssoLogoutCall);
                }
                String checkUrl = AuthSSOUtil.buildCheckTicketUrl(ticket, ssoLogoutCall);
                Object body = cfg.sendHttp.apply(checkUrl);
                loginId = (ObjectUtil.isNull(body) ? null : body);
            } else {
                // 方式2：直连Redis校验ticket 
                loginId = AuthSSOUtil.checkTicket(ticket);
            }
            // Be: 如果开发者自定义了处理逻辑 
            ...
            if(loginId != null ) {
                authLogic.login(loginId);
                return res.redirect(back);
            } else {
                // 如果ticket无效: 
                throw new AuthTokenException("无效ticket：" + ticket);
            }
        }
    }
    ...
    public static Object ssoLogoutType3() {
        ...
        String url = AuthSSOUtil.buildSloUrl(authLogic.getLoginId());
        String body = String.valueOf(cfg.sendHttp.apply(url));
        if(AuthSSOConstants.OK.equals(body)) {
            if(req.getParam(ParamName.back) == null) {
                return CommonResponse.responseOkMsg("单点注销成功");
            } else {
                return res.redirect(req.getParam(ParamName.back, "/"));
