...
import com.yun.authority.oauth2.model.*;
...
public class OAuth2Template {
    ...
    public AccessTokenModel checkAccessToken(String accessToken) {
        ...
        OAuth2Exception.throwBy(at == null, "无效access_token：" + accessToken);
        return at;
    }
    /**
    ...
    public ClientTokenModel checkClientToken(String clientToken) {
        ...
        OAuth2Exception.throwBy(ct == null, "无效：client_token" + ct);
        return ct;
    }
    /**
    ...
    public void checkScope(String accessToken, String... scopes) {
        ...
        List<String> scopeList = AuthFoxUtil.convertStringToList(at.scope);
        for (String scope : scopes) {
            OAuth2Exception.throwBy(scopeList.contains(scope) == false, "该 Access-Token 不具备 Scope：" + scope);
        }
    }
     ...
     */
    public RequestAuthModel generateRequestAuth(AuthRequest req, Object loginId) {
    ...
    public AccessTokenModel generateAccessToken(String code) {
        ...
        OAuth2Exception.throwBy(cm == null, "无效code");
    ...
    public AccessTokenModel refreshAccessToken(String refreshToken) {
        ...
        OAuth2Exception.throwBy(rt == null, "无效refresh_token: " + refreshToken);

        // 如果配置了[每次刷新产生新的Refresh-Token]
        if(OAuth2Manager.getConfig().getIsNewRefresh()) {
            // 删除旧 Refresh-Token 
    ...
    public AccessTokenModel generateAccessToken(RequestAuthModel ra, boolean isCreateRt) {
        ...
        at.expiresTime = System.currentTimeMillis() + (OAuth2Manager.getConfig().getAccessTokenTimeout() * 1000);
    ...
    public ClientTokenModel generateClientToken(String clientId, String scope) {
        ...
        ct.expiresTime = System.currentTimeMillis() + (OAuth2Manager.getConfig().getClientTokenTimeout() * 1000);
    ...
    public String buildRedirectUri(String redirectUri, String code, String state) {
        String url = AuthFoxUtil.joinParam(redirectUri, Param.code, code);
        if(StrUtil.isNotBlank(state)) {
            url = AuthFoxUtil.joinParam(url, Param.state, state);
        }
        return url;
    }
    /**
    ...
    public String buildImplicitRedirectUri(String redirectUri, String token, String state) {
        String url = AuthFoxUtil.joinSharpParam(redirectUri, Param.token, token);
        if(StrUtil.isNotBlank(state)) {
            url = AuthFoxUtil.joinSharpParam(url, Param.state, state);
        }
        return url;
    }
    /**
    ...
    public boolean isGrant(Object loginId, String clientId, String scope) {
        List<String> grantScopeList = AuthFoxUtil.convertStringToList(getGrantScope(clientId, loginId));
        List<String> scopeList = AuthFoxUtil.convertStringToList(scope);
    ...
    public void checkContract(String clientId, String scope) {
        List<String> clientScopeList = AuthFoxUtil.convertStringToList(checkClientModel(clientId).contractScope);
        List<String> scopelist = AuthFoxUtil.convertStringToList(scope);
    ...
    public void checkRightUrl(String clientId, String url) {
        ...
        // 3、是否在[允许地址列表]之中 
        List<String> allowList = AuthFoxUtil.convertStringToList(checkClientModel(clientId).allowUrl);
        if(AuthManager.getAuthTokenAction().hasElement(allowList, url) == false) {
     ...
     */
    public ClientModel checkClientSecret(String clientId, String clientSecret) {
        ClientModel cm = checkClientModel(clientId);
        OAuth2Exception.throwBy(cm.clientSecret == null || cm.clientSecret.equals(clientSecret) == false, "无效client_secret: " + clientSecret);
        return cm;
    }
    /**
    ...
    public CodeModel checkGainTokenParam(String code, String clientId, String clientSecret, String redirectUri) {
        ...
        // 校验：Code是否存在 
        CodeModel cm = getCode(code);
        OAuth2Exception.throwBy(cm == null, "无效code: " + code);

        // 校验：ClientId是否一致 
        OAuth2Exception.throwBy(cm.clientId.equals(clientId) == false, "无效client_id: " + clientId);

        // 校验：Secret是否正确 
        String dbSecret = checkClientModel(clientId).clientSecret;
        OAuth2Exception.throwBy(dbSecret == null || dbSecret.equals(clientSecret) == false, "无效client_secret: " + clientSecret);

        // 如果提供了redirectUri，则校验其是否与请求Code时提供的一致 
        if(StrUtil.isNotBlank(redirectUri)) {
            OAuth2Exception.throwBy(redirectUri.equals(cm.redirectUri) == false, "无效redirect_uri: " + redirectUri);
        }
    ...
    public RefreshTokenModel checkRefreshTokenParam(String clientId, String clientSecret, String refreshToken) {
        ...
        // 校验：ClientId是否一致 
        OAuth2Exception.throwBy(rt.clientId.equals(clientId) == false, "无效client_id: " + clientId);
    ...
    public AccessTokenModel checkAccessTokenParam(String clientId, String clientSecret, String accessToken) {
        AccessTokenModel at = checkAccessToken(accessToken);
        OAuth2Exception.throwBy(at.clientId.equals(clientId) == false, "无效client_id：" + clientId);
    ...
    public RefreshTokenModel converAccessTokenToRefreshToken(AccessTokenModel at) {
        ...
        rt.scope = at.scope;
        rt.openid = at.openid;
        rt.expiresTime = System.currentTimeMillis() + (OAuth2Manager.getConfig().getRefreshTokenTimeout() * 1000);
        // 改变at属性 
        at.refreshToken = rt.refreshToken;
        at.refreshExpiresTime = rt.expiresTime;
        return rt;
    ...
    public RefreshTokenModel converRefreshTokenToRefreshToken(RefreshTokenModel rt) {
        ...
        newRt.expiresTime = System.currentTimeMillis() + (OAuth2Manager.getConfig().getRefreshTokenTimeout() * 1000);
    ...
    public void saveCode(CodeModel c) {
        ...
        AuthManager.getAuthTokenDao().setObject(splicingCodeSaveKey(c.code), c, OAuth2Manager.getConfig().getCodeTimeout());
    }
    /**
     * 持久化：Code-索引 
     * @param c . 
    ...
    public void saveCodeIndex(CodeModel c) {
        ...
        AuthManager.getAuthTokenDao().set(splicingCodeIndexKey(c.clientId, c.loginId), c.code, OAuth2Manager.getConfig().getCodeTimeout());
    }
    /**
    ...
    public void saveAccessToken(AccessTokenModel at) {
        ...
        AuthManager.getAuthTokenDao().setObject(splicingAccessTokenSaveKey(at.accessToken), at, at.getExpiresIn());
    }
    /**
     * 持久化：AccessToken-索引 
     * @param at . 
    ...
    public void saveAccessTokenIndex(AccessTokenModel at) {
        ...
        AuthManager.getAuthTokenDao().set(splicingAccessTokenIndexKey(at.clientId, at.loginId), at.accessToken, at.getExpiresIn());
    }
    /**
    ...
    public void saveRefreshToken(RefreshTokenModel rt) {
        ...
        AuthManager.getAuthTokenDao().setObject(splicingRefreshTokenSaveKey(rt.refreshToken), rt, rt.getExpiresIn());
    }
    /**
     * 持久化：RefreshToken-索引 
     * @param rt . 
    ...
    public void saveRefreshTokenIndex(RefreshTokenModel rt) {
        ...
        AuthManager.getAuthTokenDao().set(splicingRefreshTokenIndexKey(rt.clientId, rt.loginId), rt.refreshToken, rt.getExpiresIn());
    }
    /**
    ...
    public void saveClientToken(ClientTokenModel ct) {
        ...
        AuthManager.getAuthTokenDao().setObject(splicingClientTokenSaveKey(ct.clientToken), ct, ct.getExpiresIn());
    }
    /**
     * 持久化：ClientToken-索引 
     * @param ct . 
    ...
    public void saveClientTokenIndex(ClientTokenModel ct) {
        ...
        AuthManager.getAuthTokenDao().set(splicingClientTokenIndexKey(ct.clientId), ct.clientToken, ct.getExpiresIn());
    }
    /**
     * 持久化：Past-Token-索引  
     * @param ct . 
    ...
    public void savePastTokenIndex(ClientTokenModel ct) {
        ...
        AuthManager.getAuthTokenDao().set(splicingPastTokenIndexKey(ct.clientId), ct.clientToken, ct.getExpiresIn());
    }
    /**
     * 持久化：用户授权记录 
     * @param clientId 应用id 
    ...
    public void saveGrantScope(String clientId, Object loginId, String scope) {
        if(StrUtil.isNotBlank(scope)) {
            long ttl = OAuth2Manager.getConfig().getAccessTokenTimeout();
            AuthManager.getAuthTokenDao().set(splicingGrantScopeKey(clientId, loginId), scope, ttl);
        }
    }
    ...
    public CodeModel getCode(String code) {
        ...
        return (CodeModel)AuthManager.getAuthTokenDao().getObject(splicingCodeSaveKey(code));
    }
    /**
     * 获取：Code Value 
     * @param clientId 应用id 
    ...
    public String getCodeValue(String clientId, Object loginId) {
        return AuthManager.getAuthTokenDao().get(splicingCodeIndexKey(clientId, loginId));
    }
    /**
    ...
    public AccessTokenModel getAccessToken(String accessToken) {
        ...
        return (AccessTokenModel)AuthManager.getAuthTokenDao().getObject(splicingAccessTokenSaveKey(accessToken));
    }
    /**
    ...
    public String getAccessTokenValue(String clientId, Object loginId) {
        return AuthManager.getAuthTokenDao().get(splicingAccessTokenIndexKey(clientId, loginId));
    }
    /**
    ...
    public RefreshTokenModel getRefreshToken(String refreshToken) {
        ...
        return (RefreshTokenModel)AuthManager.getAuthTokenDao().getObject(splicingRefreshTokenSaveKey(refreshToken));
    }
    /**
    ...
    public String getRefreshTokenValue(String clientId, Object loginId) {
        return AuthManager.getAuthTokenDao().get(splicingRefreshTokenIndexKey(clientId, loginId));
    }
    /**
    ...
    public ClientTokenModel getClientToken(String clientToken) {
        ...
        return (ClientTokenModel)AuthManager.getAuthTokenDao().getObject(splicingClientTokenSaveKey(clientToken));
    }
    /**
    ...
    public String getClientTokenValue(String clientId) {
        return AuthManager.getAuthTokenDao().get(splicingClientTokenIndexKey(clientId));
    }
    /**
     * 获取：Past-Token Value 
     * @param clientId 应用id 
    ...
    public String getPastTokenValue(String clientId) {
        return AuthManager.getAuthTokenDao().get(splicingPastTokenIndexKey(clientId));
    }
    /**
     * 获取：用户授权记录 
     * @param clientId 应用id 
    ...
    public String getGrantScope(String clientId, Object loginId) {
        return AuthManager.getAuthTokenDao().get(splicingGrantScopeKey(clientId, loginId));
    }
    ...
    public void deleteCode(String code) {
        if(code != null) {
            AuthManager.getAuthTokenDao().deleteObject(splicingCodeSaveKey(code));
        }
    }
    /**
     * 删除：Code索引 
    ...
    public void deleteCodeIndex(String clientId, Object loginId) {
        AuthManager.getAuthTokenDao().delete(splicingCodeIndexKey(clientId, loginId));
    }
    /**
     * 删除：Access-Token 
     * @param accessToken 值 
    ...
    public void deleteAccessToken(String accessToken) {
        if(accessToken != null) {
            AuthManager.getAuthTokenDao().deleteObject(splicingAccessTokenSaveKey(accessToken));
        }
    }
    /**
     * 删除：Access-Token索引 
    ...
    public void deleteAccessTokenIndex(String clientId, Object loginId) {
        AuthManager.getAuthTokenDao().delete(splicingAccessTokenIndexKey(clientId, loginId));
    }
    /**
     * 删除：Refresh-Token 
    ...
    public void deleteRefreshToken(String refreshToken) {
        if(refreshToken != null) {
            AuthManager.getAuthTokenDao().deleteObject(splicingRefreshTokenSaveKey(refreshToken));
        }
    }
    /**
     * 删除：Refresh-Token索引 
    ...
    public void deleteRefreshTokenIndex(String clientId, Object loginId) {
        AuthManager.getAuthTokenDao().delete(splicingRefreshTokenIndexKey(clientId, loginId));
    }
    /**
     * 删除：Client-Token 
     * @param clientToken 值 
    ...
    public void deleteClientToken(String clientToken) {
        if(clientToken != null) {
            AuthManager.getAuthTokenDao().deleteObject(splicingClientTokenSaveKey(clientToken));
        }
    }
    /**
     * 删除：Client-Token索引 
    ...
    public void deleteClientTokenIndex(String clientId) {
        AuthManager.getAuthTokenDao().delete(splicingClientTokenIndexKey(clientId));
    }
    /**
     * 删除：Past-Token索引 
     * @param clientId 应用id 
    ...
    public void deletePastTokenIndex(String clientId) {
        AuthManager.getAuthTokenDao().delete(splicingPastTokenIndexKey(clientId));
    }
    /**
     * 删除：用户授权记录 
     * @param clientId 应用id 
    ...
    public void deleteGrantScope(String clientId, Object loginId) {
        AuthManager.getAuthTokenDao().delete(splicingGrantScopeKey(clientId, loginId));
    }
    ...
    public String splicingCodeSaveKey(String code) {
        return AuthManager.getConfig().getTokenName() + ":oauth2:code:" + code;
    }
    /**
     * 拼接key：Code索引 
     * @param clientId 应用id
    ...
    public String splicingCodeIndexKey(String clientId, Object loginId) {
        return AuthManager.getConfig().getTokenName() + ":oauth2:code-index:" + clientId + ":" + loginId;
    }
    /**
    ...
    public String splicingAccessTokenSaveKey(String accessToken) {
        return AuthManager.getConfig().getTokenName() + ":oauth2:access-token:" + accessToken;
    }
    /**
    ...
    public String splicingAccessTokenIndexKey(String clientId, Object loginId) {
        return AuthManager.getConfig().getTokenName() + ":oauth2:access-token-index:" + clientId + ":" + loginId;
    }
    /**
    ...
    public String splicingRefreshTokenSaveKey(String refreshToken) {
        return AuthManager.getConfig().getTokenName() + ":oauth2:refresh-token:" + refreshToken;
    }
    /**
    ...
    public String splicingRefreshTokenIndexKey(String clientId, Object loginId) {
        return AuthManager.getConfig().getTokenName() + ":oauth2:refresh-token-index:" + clientId + ":" + loginId;
    }
    /**
    ...
    public String splicingClientTokenSaveKey(String clientToken) {
        return AuthManager.getConfig().getTokenName() + ":oauth2:client-token:" + clientToken;
    }
    /**
     * 拼接key：Past-Token 索引 
    ...
    public String splicingClientTokenIndexKey(String clientId) {
        return AuthManager.getConfig().getTokenName() + ":oauth2:client-token-indedx:" + clientId;
    }
    /**
     * 拼接key：Past-Token 索引 
     * @param clientId clientId 
    ...
    public String splicingPastTokenIndexKey(String clientId) {
        return AuthManager.getConfig().getTokenName() + ":oauth2:past-token-indedx:" + clientId;
    }
    /**
     * 拼接key：用户授权记录  
     * @param clientId 应用id
    ...
    public String splicingGrantScopeKey(String clientId, Object loginId) {
        return AuthManager.getConfig().getTokenName() + ":oauth2:grant-scope:" + clientId + ":" + loginId;
    }
